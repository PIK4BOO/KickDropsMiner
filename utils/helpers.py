...
def _migrate_legacy_source_data(resource_dir, data_dir):
    """Move data created by older source runs that used utils/ as the app dir."""
    if getattr(sys, "frozen", False):
        return

    legacy_dir = os.path.join(resource_dir, "utils")
    if legacy_dir == data_dir or not os.path.isdir(legacy_dir):
        return

    src_config = os.path.join(legacy_dir, "config.json")
    dst_config = os.path.join(data_dir, "config.json")
    if os.path.exists(src_config) and not os.path.exists(dst_config):
        try:
            shutil.copy2(src_config, dst_config)
    ...
    for folder in ("cookies", "chrome_data"):
        src = os.path.join(legacy_dir, folder)
        dst = os.path.join(data_dir, folder)
        if not os.path.isdir(src):
            continue
        try:
...
_migrate_legacy_source_data(APP_DIR, DATA_DIR)
COOKIES_DIR = os.path.join(DATA_DIR, "cookies")
CONFIG_FILE = os.path.join(DATA_DIR, "config.json")
CHROME_DATA_DIR = os.path.join(DATA_DIR, "chrome_data")
LOCK_FILE = os.path.join(DATA_DIR, "kickdropsminer.lock")
...
def lock_file(path):
    """Try to lock a file and return (success, file_handle).

    Keep the returned handle open for as long as the lock should live.
    """
    file = open(path, "w", encoding="utf-8")
    file.write("KickDropsMiner\n")
    file.flush()
    if sys.platform == "win32":
        import msvcrt

        try:
            msvcrt.locking(file.fileno(), msvcrt.LK_NBLCK, max(os.path.getsize(path), 1))
        except OSError:
            return False, file
        return True, file
