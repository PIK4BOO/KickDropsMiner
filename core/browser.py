...
class CookieManager:
    ...
    def save_cookies(driver, domain):
        ...
        try:
            ...
            all_cookies = data.get("cookies", []) if isinstance(data, dict) else []
    ...
    @staticmethod
    def has_saved_session(domain):
        cookies = CookieManager.saved_cookies(domain)
    ...
    @staticmethod
    def load_cookies_via_cdp(driver, domain):
        """Load saved cookies without first navigating to the site."""
        path = cookie_file_for_domain(domain)
        if not os.path.exists(path):
            return False
        try:
        ...
        for c in cookies:
            ...
            if expiry is not None:
                try:
                    ...
                    pass
            if "httpOnly" in c:
                params["httpOnly"] = bool(c.get("httpOnly"))
            try:
...
def make_chrome_driver(
    ...
    user_data_dir = user_data_dir or CHROME_DATA_DIR
