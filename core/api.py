...
from .kick_public_api import KickPublicAPI, livestream_to_streamer


_PUBLIC_API = None
_PUBLIC_API_SAVE_CALLBACK = None


def configure_public_api(
    client_id="",
    client_secret="",
    access_token="",
    expires_at=0,
    ...
    """Configure the optional official Kick Public API client."""
    global _PUBLIC_API, _PUBLIC_API_SAVE_CALLBACK
    _PUBLIC_API = KickPublicAPI(client_id, client_secret, access_token, expires_at)
    _PUBLIC_API_SAVE_CALLBACK = save_callback
    return _PUBLIC_API


def public_api_available():
    return bool(_PUBLIC_API and _PUBLIC_API.configured())
...
def _sync_public_api_token():
    if _PUBLIC_API and _PUBLIC_API_SAVE_CALLBACK:
        try:
            _PUBLIC_API_SAVE_CALLBACK(_PUBLIC_API.access_token, _PUBLIC_API.expires_at)
        except Exception:
            pass


def persist_public_api_token():
    """Persist the public API token after any lazy refresh."""
    _sync_public_api_token()
...
def kick_live_status_by_api(url: str):
    ...
    try:
        ...
        if public_api_available():
            try:
                status = _PUBLIC_API.live_status_by_slug(username)
                _sync_public_api_token()
                if status is not None:
                    return status
            except Exception as e:
                debug_print(f"DEBUG: Public API live check failed for {username}: {e}")
...
def kick_channel_category_id_by_api(url: str):
    """Return the current livestream category id for a Kick channel, or None when unknown/offline."""
    try:
        p = urlparse(url)
        if "kick.com" not in p.netloc:
            return None
        ...
        if public_api_available():
            try:
                category_id = _PUBLIC_API.category_id_by_slug(username)
                _sync_public_api_token()
                if category_id is not None:
                    return category_id
            except Exception as e:
                debug_print(f"DEBUG: Public API category check failed for {username}: {e}")
        ...
        category = livestream.get("category") or data.get("category")
...
def fetch_live_streamers_by_category(category_id, limit=24, driver=None):
    ...
    if public_api_available():
        try:
            streams = _PUBLIC_API.get_livestreams_by_category(category_id, limit=limit)
            _sync_public_api_token()
            streamers = []
            for stream in streams[:limit]:
                item = livestream_to_streamer(stream)
                if item:
                    streamers.append(item)
            if streamers:
                debug_print(f"DEBUG: Public API returned {len(streamers)} streamers")
                return streamers
        except Exception as e:
            debug_print(f"DEBUG: Public API category streamers failed: {e}")
...
def fetch_drops_progress_direct():
    """Fetch current drop progress using saved cookies only.

    This never creates a Chrome driver, so it is safe for lightweight status
    refreshes while the miner worker is already using the browser profile.
    """
    api_url = f"https://web.kick.com/api/v1/drops/progress?_={int(time.time())}"
    response = _fetch_kick_json(api_url, authenticated=True)
    progress_data = response.get("data", []) if isinstance(response, dict) else []
    return {"progress": progress_data, "driver": None}
